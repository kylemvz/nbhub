...
class Marathon:
    ...
    def start_container(self,
        ...
        del new_container['docker']['portMappings']
        logger.warning("Ports: %s" % ports) 
        #new_container['docker']['ports'] = [int(port) for port in ports]
        #new_request['requirePorts'] = True
        new_request['ports'] = [int(port) for port in ports]
        #new_request['portDefinitions'] = [{"port": int(port)} for port in ports]

        new_container['docker']['network'] = network_mode
        new_request['container'] = new_container
        pprint.pprint(new_request)
        logger.warning("%s" % new_request)
